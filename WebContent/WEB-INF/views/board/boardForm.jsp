...
<div>
	<section class="roomDetail-info">
		<div id="roomDetailView1">
			<%=room.getRoom_val().contains("T") ? "투룸" : "원룸"%><br /> <span
				id="roomDetailTxt1"><%=room.getTax_val().equals("M") ? "월세" : "전세"%>
				<%=room.getPrice()%></span>만원
		</div>
		<div id="roomDetailView2">
			전용면적<br /> <span id="roomDetailTxt2"><%=room.getSize()%></span>평
		</div>
		<div id="roomDetailView3">
			관리비<br /> <span id="roomDetailTxt1"><%=room.getTax_val().equals("M") ? "월세" : "전세"%>
				<%=room.getPrice()%></span>만원
		</div>
		...
		<div id="roomDetailView6">
			<form action="<%=request.getContextPath()%>/userChat/chatView" method="post" name="chatViewForm">
				<input type="hidden" name="toID" value="<%=br.getBr_cp_id()%>">
				<button type="submit" class="brokerChatBtn">채팅하기</button>
			</form>
		</div>
		...
		<div id="connectWrap">
			...
			<input type="button" class="closeShowBrokerBtn" value="x"
				onclick="brokerCloseBtn();">

			<div
				style="padding: 0 0 30px 40px; margin-bottom: 30px; border-bottom: 1px solid rgb(235, 235, 235);">
			...
			<div>
				<div class="profileImg">
					<img id="profileImg"
						src="<%=request.getContextPath()%>/upload/member/<%=br.getProfile()%>"
						onerror="this.src='<%=request.getContextPath()%>/upload/member/user.png'"
						alt="profile">
					<p><%=br.getBr_name()%>(중개 보조원)
					</p>
					<p>
						보증보험 :
						<%=br.getInsurance().equals("T") ? "유" : "무"%></p>
				</div>
			</div>
		</div>
	</section>
...
<!-- 좋아요 -->
<form name="likeFrm"
	action="<%=request.getContextPath()%>/board/boardView" method="POST">
...
<div id="certification">
	<div class="certification-frmWrap">
		...
		<input type="button" class="closeBtn" value="x"
			onclick="roomReportcloseBtn();">

		<form action="<%=request.getContextPath()%>/board/report"
			id="cerFrm" method="POST" class="input-group">
			<h3>허위매물 신고</h3>

			<div class="social-icons">
				<img src="<%=request.getContextPath()%>/images/alert.png"
					alt="alarm">
			</div>
			...
			<input type="hidden" name="br_cp_id" id="cerHidden"
				value="<%=br.getBr_cp_id()%>" /> <input type="hidden"
				name="board_num" id="cerHidden2"
				value="<%=roomBoard.getBoard_num()%>" /> <input type="email"
				id="cerEmail" name="cerEmail" class="cer-field"
				placeholder="abc@naver.com" required> <input type="button"
				class="sendMail" value="전송" /> <br /> <input type="text"
				id="cerNumber" name="cerNumber" class="cer-field"
				placeholder="인증번호를 입력해주세요" required> <input type="submit"
				class="submit" id="cerSubmit" value="인증" />
		</form>
	</div>
</div>
...
<div class="roomDetail-info3">
	<section id="detail-infoTitle">
		<p class="detail-info1"><%=roomBoard.getOk().equals("F") ? "비 확인 매물" : "확인 매물"%>
			<%=roomBoard.getEnrolldate()%></p>
...
<div class="roomDetail-info4">
	...
	%>
	<div>
		<img class="roomDetailImg01"
			src="<%=request.getContextPath()%>/upload/board/<%=ri.getRenameName()%>"
			alt=""
...
<div class="roomDetail-info6">
	...
	<input type="text" id="roomLocation" value="<%=room.getLocation()%>"
		readonly />
...
<div class="roomReview">
	...
	<form name="reviewform" class="reviewform" method="post"
		action="<%=request.getContextPath()%>/board/insertreview">
		<input type="hidden" name="rate" id="rate" value="0" /> <input
			type="hidden" id="revBrId" name="boardno"
			value="<%=roomBoard.getBoard_num()%>" /> <input type="hidden"
			id="revBrcpId" name="br_cp_id" value="<%=roomBoard.getBr_cp_id()%>" />
		<input type="hidden" id="revMemId" name="memberId"
			value="<%=memberLoggedIn.getMemberId()%>" /> <input type="hidden"
			id="revRoomNum" name="roomnum" value="<%=room.getRoom_num()%>" />
		...
		<div class="review_contents">
			...
			<textarea rows="10" name="reviewcontent"
				placeholder="두 글자 이상 작성해 주세요." class="review_textarea"></textarea>
		</div>
		<div class="cmd">
...
<table id="tbl-review">
	...
	%>
	<tr class="review">
		<td class="reviewMemProfileIdDate"><sub
			class="coveredMemberProfileWriterDate"> <sub
				class="review-memberProfile"> <img
					class="reviewMemberProfileImg"
					src="<%=request.getContextPath()%>/images/user.png"></sub> <sub
				class="review-writer" name="userId"> <%=rr.getUserId()%></sub> <sub
				class="review-date" name="reviewenrolldate"> <%=rr.getEnrolldate()%></sub>
		</sub> <br /></td>

		<td class="reviewContentAndDelete"><sub class="review-rating">
				<script>
								function showRating()
								{
									<%switch (rr.getRating()) {
		...
		</sub> <sub class="ReviewMainContent">
				...
				<%
					if (memberLoggedIn != null && (rr.getUserId().equals(memberLoggedIn.getMemberId())
									|| memberLoggedIn.getMemberRole().equals(MemberService.MEMBER_ROLE_ADMIN))) {
				%>

				<form class="reviewupdate"
					action="<%=request.getContextPath()%>/board/reviewupdate"
					method="POST" name="reviewupdate">
					<button class="Reviewbtn-update" value="<%=rr.getReviewId()%>"
						onclick="reviewupdate();" class="update">수정</button>
					<input type="hidden" name="reviewno" value=<%=rr.getReviewId()%> />
					<input type="hidden" id="revBrId" name="boardno"
						value="<%=roomBoard.getBoard_num()%>" /> <input type="hidden"
						id="revBrcpId" name="br_cp_id"
						value="<%=roomBoard.getBr_cp_id()%>" /> <input type="hidden"
						id="revMemId" name="memberId"
						value="<%=memberLoggedIn.getMemberId()%>" /> <input
						type="hidden" id="revRoomNum" name="roomno"
						value="<%=room.getRoom_num()%>" /> <input type="hidden"
						id="revRating" name="rating" value="<%=rr.getRating()%>" />
				</form>

				<form class="reviewdelete"
					action="<%=request.getContextPath()%>/board/reviewdelete"
					method="POST" name="reviewdelete">
					<button class="Reviewbtn-delete"
						value="<%=roomBoard.getBoard_num()%>" onclick="reviewdelete();"
...
//주소로 좌표를 검색합니다
geocoder.addressSearch('<%=room.getLocation()%>',
