...
<div class="roomUpdatePage">
	...
	<form action="<%=request.getContextPath()%>/board/enroll" method="post"
		...
		<input type="hidden" name="br_cp_id"
			value="<%=memberLoggedIn.getMemberId()%>" />
		<section>
			<input type="text" name="board_title" id="boardTitle"
		...
		<section id="roomUpSection">
			...
			</select> <select name="size" class="selectOption">
				<option value="14">14평</option>
				<option value="24">24평</option>
				<option value="34">34평</option>
			</select> <span> 입주가능일 : <input type="date" name="movedate"
				id="movedate">
			</span> <span> 층 : <input type="text" name="floor" id="floor">
			</span>
		</section>
		...
		<section>
			<div id="roomUploadFile">
				...
				<!-- 위치 검색 -->
				<div
					style="width: 75%; height: 450px; margin-top: 40px; border-bottom: 1px solid #999;">
					<div id="roomUpSearch">
						<p id="roomUpSearchTxt">방 위치 선택</p>
						<input type="text" name="location" id="localSearch"
							placeholder="방 위치를 입력해주세요" required>
						<button onclick="searchPlaces(); return false;"
							class="room-searchBtn">확인</button>
						<br /> <br />
						<p>주의사항</p>
						<br />
						<p>상세한 주소를 검색해주세요!</p>
				...
				<div id="roomUpFile">
					...
					<div id="roomUpFileInput">
						<input type="file" name="f0" /> &nbsp;&nbsp; <input type="button"
...
<script>
	...
	var idx = 0;
	function addFile() {
		$("#roomUpFileInput").last().after(
				"<input type='file' name='f"+ ++idx +"'/><br>");
	}
	function delFile() {
