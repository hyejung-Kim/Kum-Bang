...
<body>
		...
		<div class="navbar-collapse" id="navbarResponsive">
			<ul class="navbar-nav">
				...
				<li class="nav-item-room"><a
					...
					class="nav-link">채팅목록<span id="unread" class="label label-info"></span></a></li>
				...
				<li class="nav-item"><a href="#" class="nav-user-link"><img
						style="background-color: rgb(255, 136, 81); border-radius: 60%;"
				...
				<ul class="profile__list">
					...
					<li class="login-mypage" id="login-mypage-info"><div
							onclick="location.href='<%=request.getContextPath()%>/member/memberProfileView'">내
							정보 보기</div>
					<li class="login-mypage" id="login-mypage-logout"><div
							onclick="location.href='<%=request.getContextPath()%>/member/logout'">로그아웃</div></li>
				</ul>
				<%
					} else {
				%>
				...
				<li class="nav-item-room"><a
					...
					class="nav-link">방 찾기</a></li>
				<li class="nav-item"><a href="#" class="nav-user-link"><img
						style="background-color: rgb(0, 0, 255); border-radius: 60%;"
						src="<%=request.getContextPath()%>/images/user.png" /></a></li>
				...
				<ul class="profile__list">
					...
					<li class="login-profile" id="login-profile-email"><%=memberLoggedIn.getEmail()%></li>
					<li class="login-mypage" id="login-mypage-info"><div
							onclick="location.href='<%=request.getContextPath()%>/admin/memberList';">회원정보
							조회</div></li>
					<li class="login-mypage" id="login-mypage-logout"><div
							onclick="location.href='<%=request.getContextPath()%>/member/logout'">로그아웃</div></li>
				</ul>
	...
	<div id="wrap">
		<div class="form-wrap">
			...
			<input type="button" class="HeaderCloseBtn" id="closeBtn" value="x"
				onclick="closeBtn();">

			<form action="<%=request.getContextPath()%>/member/login" id="login"
				...
				<%--<input type="text" id="loginId" name="memberId" class="input-field"
					placeholder="아이디를 입력해주세요" required
					value="<%=saveIdChecked ? saveIdValue : ""%>" /> --%>
				<input type="password" id="loginPwd" name="password" class="input-field"
					placeholder="비밀번호를 입력해주세요" required>
				<div id="chkWrap">
					<input type="checkbox" class="checkbox" name="saveId"
						<%=saveIdChecked ? "checked" : ""%> />아이디 저장 <span
						class="checkbox brokerChk" id="br_frm"
						onclick="location.href='<%=request.getContextPath()%>/broker/index'">중개인이세요?</span>
				</div>
				<input type="submit" class="submit" value="로그인" /> <a id="findLink"
					onclick="findLink()">비밀번호 찾기</a>
			</form>
			<form id="register"
				action="<%=request.getContextPath()%>/member/enroll" method="post"
				onsubmit="return resisterVal();" class="input-group">
				<input type="text" id="enrollUserId" name="userID" class="input-field"
	...
	<div id="find_wrap">
		<div class="findFrm-wrap">
			<input type="button" id="find_closeBtn" value="x"
				onclick="find_closeBtn();">
			<p>비밀번호 찾기</p>
			<form action="<%=request.getContextPath()%>/member/findPwd"
				id="find-pwd" method="post" class="input-group">
				<input type="text" id="input-email" name="email" class="input-field"
					placeholder="이메일을 입력해주세요" required /> <input type="submit"
					class="findId-submit" value="이메일 전송" />
			</form>
		</div>
	</div>
	<script>
